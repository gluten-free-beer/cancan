    ...
    if (typeof _w.cancan !== "undefined") {
        ...
        function play(queue, wrapper) {
            if (queue.length && playstate) {
                ...
                const {thumbnail = null, context = null, title, boid, url: ourl = "", provider = null} = bobj;
                // change to all to provide info even if some fields are missing
                if (thumbnail && context) {
                    let url = ourl;
                    if (provider && provider in ref && boid in ref[provider]) {
                        url = ref[provider][boid]; // to replace original product url with your own affiliate link, use automator to batch process if needed: https://github.com/gluten-free-beer/cancan_affiliate_link_automate
                    }
                    let iname = title;
                    if (url.length > 10) {
